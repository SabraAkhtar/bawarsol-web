...
export const PricingPage: React.FC<PricingPageProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#F0F0F0] pt-24 pb-12 overflow-hidden">
      ...
      {/* Hero */}
      <section className="relative py-20 border-b border-white/10 hero-radial-bg text-center">
        <motion.div 
          initial="hidden"
          whileInView="visible"
        ...
        >
          ...
          <motion.p variants={itemVariants} className="text-slate-300 text-base sm:text-xl max-w-2xl mx-auto leading-relaxed">
      ...
      <section className="py-20 relative">
        <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-3/4 h-1/2 bg-[#00F0FF]/5 rounded-full blur-[150px] pointer-events-none" />
        ...
        >
            ...
            {plans.map((plan) => (
              <motion.div
                ...
                className={`relative p-8 rounded-[2rem] flex flex-col justify-between transition-all duration-300 shadow-2xl group overflow-hidden ${
                  plan.popular
                    ? 'bg-white/[0.04] backdrop-blur-2xl border-2 border-[#00F0FF] shadow-[0_0_40px_rgba(0,240,255,0.15)] md:-mt-4 md:mb-4 relative z-20'
                    : 'bg-white/[0.02] backdrop-blur-2xl border border-white/5 hover:border-white/20 hover:bg-white/[0.04]'
                }`}
              >
                {plan.popular && (
                ...
                {plan.popular && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-30">
                    <span className="px-4 py-1.5 rounded-full text-xs font-bold bg-[#00F0FF] text-black shadow-lg shadow-[#00F0FF]/40 uppercase tracking-wider">
                      Most Popular
                    </span>
                  </div>
                )}
                ...
                <div className="space-y-6 relative z-10">
                  <div>
                    <h3 className={`text-2xl font-extrabold ${plan.popular ? 'text-white' : 'text-slate-200 group-hover:text-white transition-colors'}`}>{plan.name}</h3>
                  ...
                  <div className="flex items-end gap-1.5 border-b border-white/10 pb-6">
                    <span className={`text-4xl sm:text-5xl font-extrabold tracking-tight ${plan.popular ? 'text-[#00F0FF] drop-shadow-[0_0_15px_rgba(0,240,255,0.4)]' : 'text-white'}`}>
                      {plan.price}
                    </span>
                    {plan.period && (
                      <span className="text-slate-400 text-sm mb-2 font-medium">{plan.period}</span>
                    )}
                  </div>
                  ...
                  <ul className="space-y-3 pt-2">
                    {plan.features.map((f) => (
                      <li key={f} className="flex items-start gap-3 text-sm text-slate-300">
                        <CheckCircle2 className={`w-4 h-4 shrink-0 mt-0.5 ${plan.popular ? 'text-[#00F0FF]' : 'text-slate-500 group-hover:text-[#00F0FF]/50 transition-colors'}`} />
                        <span>{f}</span>
                      </li>
                    ))}
                  </ul>
                ...
                <div className="pt-8 relative z-10">
                  <button
                    ...
                    className={`w-full py-4 rounded-full font-bold text-sm transition-all flex items-center justify-center gap-2 ${
                      plan.popular
                        ? 'bg-[#00F0FF] text-black hover:bg-[#33F3FF] shadow-[0_0_20px_rgba(0,240,255,0.3)] hover:shadow-[0_0_30px_rgba(0,240,255,0.5)]'
                        : 'bg-white/[0.05] text-white border border-white/10 hover:border-white/30 hover:bg-white/10'
                    }`}
                  >
          ...
          {/* FAQ / Note */}
          <motion.div variants={itemVariants} className="mt-24 p-8 sm:p-12 rounded-[2rem] bg-white/[0.02] backdrop-blur-2xl border border-white/10 space-y-10 shadow-2xl relative overflow-hidden">
            <div className="absolute inset-0 bg-gradient-to-br from-[#00F0FF]/[0.02] to-transparent pointer-events-none" />
            
            <div className="text-center relative z-10">
              <div className="inline-flex items-center justify-center p-3 rounded-2xl bg-[#00F0FF]/10 text-[#00F0FF] mb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
              {[
                ...
                <div key={q} className="space-y-3 p-6 rounded-2xl bg-black/40 border border-white/5 hover:border-white/10 transition-colors shadow-inner">
                  <h4 className="text-base font-bold text-white flex items-start gap-2">
                    <span className="text-[#00F0FF] mt-1">•</span>
                    {q}
                  </h4>
                  <p className="text-sm text-slate-400 leading-relaxed pl-4">{a}</p>
                </div>
              ))}
            </div>
          </motion.div>
      ...
      <section className="py-20 border-t border-white/10 relative">
        <div className="absolute top-1/2 right-0 w-1/2 h-1/2 bg-[#00F0FF]/5 rounded-full blur-[120px] pointer-events-none" />
        ...
        >
          <motion.div variants={itemVariants} className="text-center mb-12">
            <h3 className="text-3xl font-extrabold text-white">What Our Clients Say</h3>
            <p className="text-slate-400 text-base mt-4 max-w-2xl mx-auto">Don't just take our word for it. Hear from the leaders who have transformed their digital presence with BawarSol.</p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <motion.div variants={itemVariants} key={name} className="p-8 rounded-[2rem] bg-white/[0.02] backdrop-blur-xl border border-white/5 hover:border-white/10 hover:bg-white/[0.04] transition-all space-y-6 flex flex-col justify-between group shadow-xl">
                <div className="text-[#00F0FF] opacity-50 mb-2">
                ...
                <p className="text-slate-300 text-sm leading-relaxed font-medium italic relative z-10 flex-grow">
                  {quote}
                </p>
                <div className="flex items-center gap-4 pt-4 border-t border-white/5">
                  <div className="w-10 h-10 rounded-full bg-[#00F0FF]/10 flex items-center justify-center text-[#00F0FF] font-bold text-sm border border-[#00F0FF]/20 group-hover:bg-[#00F0FF]/20 transition-colors">
                    {name.charAt(0)}
                  </div>
                  <div>
                    <div className="text-sm font-bold text-white group-hover:text-[#00F0FF] transition-colors">{name}</div>
