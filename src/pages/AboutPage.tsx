...
export const AboutPage: React.FC<AboutPageProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#F0F0F0] pt-24 pb-12">
      ...
      {/* Hero */}
      <section className="relative py-16 border-b border-white/10 hero-radial-bg text-center overflow-hidden">
        <motion.div 
          initial="hidden"
          whileInView="visible"
        ...
        >
          ...
          <motion.p variants={itemVariants} className="text-slate-300 text-base sm:text-lg max-w-3xl mx-auto leading-relaxed">
          ...
          <motion.div variants={itemVariants} className="pt-6 grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl mx-auto">
            <div className="p-5 rounded-2xl bg-white/[0.03] backdrop-blur-xl border border-white/10 hover:border-[#00F0FF]/30 transition-all text-left space-y-2 group">
      ...
      <section className="py-14 border-b border-white/10">
        ...
        >
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {stats.map((s) => (
              <motion.div variants={itemVariants} key={s.label} className="p-6 rounded-2xl bg-white/[0.03] backdrop-blur-xl border border-white/10 hover:border-[#00F0FF]/20 transition-all text-center space-y-2">
      ...
      <section className="py-20 relative">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3/4 h-3/4 bg-[#00F0FF]/5 rounded-full blur-[120px] pointer-events-none" />
        <motion.div 
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, margin: "-100px" }}
        ...
        >
          <div className="text-center space-y-3">
            <motion.span variants={itemVariants} className="text-xs font-bold text-[#00F0FF] uppercase tracking-widest block">What Drives Us</motion.span>
            <motion.h2 variants={itemVariants} className="text-3xl sm:text-4xl font-extrabold text-white">Our Core Values</motion.h2>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {coreValues.map((v) => (
              <motion.div variants={itemVariants} key={v.title} className="p-8 rounded-3xl bg-white/[0.03] backdrop-blur-xl border border-white/10 hover:border-[#00F0FF]/40 transition-all shadow-xl hover:-translate-y-1 group space-y-4">
                <div className="w-14 h-14 rounded-2xl bg-white/[0.05] border border-white/10 flex items-center justify-center group-hover:bg-[#00F0FF]/10 group-hover:border-[#00F0FF]/40 transition-all">
                  {v.icon}
                </div>
                <h3 className="text-xl font-bold text-white group-hover:text-[#00F0FF] transition-colors">{v.title}</h3>
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="space-y-6">
              <motion.span variants={itemVariants} className="text-xs font-bold text-[#00F0FF] uppercase tracking-widest block">Our Story</motion.span>
              <motion.h2 variants={itemVariants} className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-white leading-tight">The Journey</motion.h2>
              ...
              <motion.div variants={itemVariants} className="pt-2">
                <button
                  ...
                  className="inline-flex items-center gap-2 px-8 py-3.5 rounded-full font-bold text-sm text-black bg-[#00F0FF] hover:bg-[#33F3FF] transition-all shadow-lg shadow-[#00F0FF]/20"
                >
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-3 gap-4">
              {[
                ...
                <div key={i} className={`aspect-square rounded-2xl overflow-hidden border border-white/10 ${i % 2 === 0 ? 'translate-y-4' : '-translate-y-4'}`}>
                  <img
                    src={src}
                    alt={`Project ${i + 1}`}
      ...
      <section className="py-20">
        ...
        >
          <div className="text-center space-y-3">
            <motion.span variants={itemVariants} className="text-xs font-bold text-[#00F0FF] uppercase tracking-widest block">How We Work</motion.span>
            <motion.h2 variants={itemVariants} className="text-3xl sm:text-4xl font-extrabold text-white">Our Proven Process</motion.h2>
            <motion.p variants={itemVariants} className="text-slate-400 text-base max-w-xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {process.map((p) => (
              <motion.div variants={itemVariants} key={p.step} className="p-6 rounded-3xl bg-white/[0.03] backdrop-blur-xl border border-white/10 hover:border-[#00F0FF]/40 transition-all group space-y-4 hover:-translate-y-1">
                <div className="w-12 h-12 rounded-2xl bg-[#00F0FF]/10 border border-[#00F0FF]/30 flex items-center justify-center text-[#00F0FF] font-extrabold text-base group-hover:bg-[#00F0FF] group-hover:text-black transition-colors">
                  {p.step}
                </div>
                <h3 className="font-bold text-lg text-white group-hover:text-[#00F0FF] transition-colors">{p.title}</h3>
      ...
      <section className="py-20 border-t border-white/10">
        ...
        >
          <div className="text-center space-y-3">
            <motion.span variants={itemVariants} className="text-xs font-bold text-[#00F0FF] uppercase tracking-widest block">The Difference</motion.span>
            <motion.h2 variants={itemVariants} className="text-3xl sm:text-4xl font-extrabold text-white">Why Choose BawarSol?</motion.h2>
          </div>

          <motion.div variants={itemVariants} className="overflow-x-auto rounded-3xl border border-white/10 bg-white/[0.02] backdrop-blur-xl">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-white/[0.04] border-b border-white/10">
                  <th className="p-6 text-left text-slate-300 font-bold uppercase tracking-wider text-xs">Feature</th>
                  <th className="p-6 text-center text-slate-400 font-bold uppercase tracking-wider text-xs">Typical Agencies</th>
                  <th className="p-6 text-center text-[#00F0FF] font-bold uppercase tracking-wider text-xs">BawarSol.ai</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/10">
                {[
                  ...
                  <tr key={feature} className="hover:bg-white/[0.04] transition-colors">
                    <td className="p-6 font-semibold text-white whitespace-nowrap">{feature}</td>
                    <td className="p-6 text-center text-slate-400">{typical}</td>
                    <td className="p-6 text-center text-[#00F0FF] font-semibold bg-[#00F0FF]/[0.02]">{bawarsol}</td>
                  </tr>
                ))}
              </tbody>
            </table>
